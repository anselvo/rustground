...
pub fn level_order(root: Option<Rc<RefCell<TreeNode>>>) -> Vec<Vec<i32>> {
    ...
    while !stack.is_empty() {
        ...
        if ans.len() % 2 == 0 {
            ...
            for _ in 0..stack.len() {
                let el = stack.pop_back().unwrap();
                let el_r = el.borrow_mut();
                vec.push(el_r.val);
                if let Some(r) = &el_r.right { stack.push_front(r.clone()); }
                if let Some(l) = &el_r.left { stack.push_front(l.clone()); }
            }
        }
        ans.push(vec);
    }
...
fn test() {
    ...
    assert_eq!(level_order(two), vec![vec![3], vec![20, 9], vec![15, 7]]);
    ...
    assert_eq!(level_order(one), vec![vec![3], vec![9], vec![9], vec![9], vec![9], vec![3, 9], vec![15, 1]]);
